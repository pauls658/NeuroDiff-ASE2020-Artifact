...
int main( int argc, char *argv[])
{
    ...
    for (int i=1;i<argc;i++) {
        ...
        if (i == 5) {
            RUN_TO_DEPTH = atoi(argv[i]);

            if (RUN_TO_DEPTH != 0 && RUN_TO_DEPTH != 1) {
    ...
    // load the input intervals, and put them into matrixes
    ...
    // Do something related to "features". feature_range_length is the number of
    // inputs that have an actual interval, i.e. upperbound != lowerbound
    int n = 0;
    ...
    // Now save the indexes in the input that are part of
    // the feature_range
