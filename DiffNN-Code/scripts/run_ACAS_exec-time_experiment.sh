...
if [[ -f $out ]]; then
	...
	echo "Corwardly refusing to overwrite it"
	exit 1
fi
...
for prop in "${!exps[@]}"; do
	for compressed_nnet in compressed_nnets/ACAS*; do
		...
		if [[ "$model" =~ ${exps[$prop]} && ! "$model" =~ ${skips[$prop]} ]]; then
			...
			echo "./delta_network_test $prop $orig_nnet $compressed_nnet $epsilon -e $EXTRA_VARS" >> $out
			timeout -s 2 $TIMEOUT ./delta_network_test $prop $orig_nnet $compressed_nnet $epsilon -e $EXTRA_VARS 2>> $out
		fi
	done
done
